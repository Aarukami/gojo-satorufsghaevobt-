...
# Their work is Highly Appreciated by The gojo Team
...
import gojoAnimeBot.modules.sql.feds_sql as sql
from gojoAnimeBot import (EVENT_LOGS, LOGGER, OWNER_ID, DRAGONS, TIGERS, WOLVES,
                          dispatcher)
from gojoAnimeBot.modules.disable import DisableAbleCommandHandler
from gojoAnimeBot.modules.helper_funcs.alternate import send_message
from gojoAnimeBot.modules.helper_funcs.chat_status import is_user_admin
from gojoAnimeBot.modules.helper_funcs.extraction import (extract_unt_fedban,
                                                          extract_user,
                                                          extract_user_fban)
from gojoAnimeBot.modules.helper_funcs.string_handling import markdown_parser
    ...
    if args:
        if args[0] == 'json':
            ...
            with BytesIO(str.encode(backups)) as output:
                output.name = "gojo_fbanned_users.json"
                update.effective_message.reply_document(
                    document=output,
                    filename="gojo_fbanned_users.json",
            ...
            for users in getfban:
                ...
                backups += "\n"
            with BytesIO(str.encode(backups)) as output:
                output.name = "gojo_fbanned_users.csv"
                update.effective_message.reply_document(
                    document=output,
                    filename="gojo_fbanned_users.csv",
