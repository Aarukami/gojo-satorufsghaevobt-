...
from gojoAnimeBot import (DEV_USERS, OWNER_ID, DRAGONS, DEMONS, TIGERS, WOLVES,
                          INFOPIC, dispatcher, sw)
from gojoAnimeBot.__main__ import STATS, TOKEN, USER_INFO
import gojoAnimeBot.modules.sql.userinfo_sql as sql
from gojoAnimeBot.modules.disable import DisableAbleCommandHandler
from gojoAnimeBot.modules.sql.global_bans_sql import is_user_gbanned
from gojoAnimeBot.modules.sql.afk_sql import is_afk, check_afk_status
from gojoAnimeBot.modules.sql.users_sql import get_user_num_chats
from gojoAnimeBot.modules.helper_funcs.chat_status import sudo_plus
from gojoAnimeBot.modules.helper_funcs.extraction import extract_user
from gojoAnimeBot import telethn as gojoTelethonClient
...
def info(update: Update, context: CallbackContext):
    ...
    if user.id == OWNER_ID:
        text += "\n\nThe Ranking of The Person is Owner of gojo"
        disaster_level_present = True
    elif user.id in DEV_USERS:
        text += "\n\nThis Person is a Developer of gojo"
        disaster_level_present = True
    elif user.id in DRAGONS:
        text += "\n\nThis Person is a Sudo-User of gojo"
        disaster_level_present = True
    elif user.id in DEMONS:
        text += "\n\nThis Person is a Support User of gojo"
        disaster_level_present = True
    elif user.id in TIGERS:
        text += "\n\nThis User is Immune to all Restrictions over gojo"
        disaster_level_present = True
