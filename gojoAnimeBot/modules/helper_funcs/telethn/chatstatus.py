from gojoAnimeBot.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
from gojoAnimeBot import DRAGONS
...
async def gojo_is_admin(chat_id: int):
    status = False
    gojo = await telethn.get_me()
    async for user in telethn.iter_participants(
            chat_id, filter=ChannelParticipantsAdmins):
        if gojo.id == user.id:
            status = True
