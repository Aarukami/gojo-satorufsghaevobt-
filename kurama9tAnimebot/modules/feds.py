...
# Their work is Highly Appreciated by The kurama9t Team
...
import kurama9tAnimeBot.modules.sql.feds_sql as sql
from kurama9tAnimeBot import (EVENT_LOGS, LOGGER, OWNER_ID, DRAGONS, TIGERS, WOLVES,
                          dispatcher)
from kurama9tAnimeBot.modules.disable import DisableAbleCommandHandler
from kurama9tAnimeBot.modules.helper_funcs.alternate import send_message
from kurama9tAnimeBot.modules.helper_funcs.chat_status import is_user_admin
from kurama9tAnimeBot.modules.helper_funcs.extraction import (extract_unt_fedban,
                                                          extract_user,
                                                          extract_user_fban)
from kurama9tAnimeBot.modules.helper_funcs.string_handling import markdown_parser
...
def new_fed(update: Update, context: CallbackContext):
    ...
    if chat.type != "private":
        update.effective_message.reply_text(
            "meow Federations can only be created by privately messaging me.")
        return
    if len(message.text) == 1:
        send_message(update.effective_message,
                     "meow Please write the name of the federation!")
        return
    ...
    if not fednam == '':
        ...
        update.effective_message.reply_text("*meow You have succeeded in creating a new federation!*"\
                 "\nName: `{}`"\
                 "\nID: `{}`"
...
def del_fed(update: Update, context: CallbackContext):
    ...
    if chat.type != "private":
        update.effective_message.reply_text(
            "meow Federations can only be deleted by privately messaging me.")
        return
    if args:
        is_fed_id = args[0]
        ...
        if getinfo is False:
            update.effective_message.reply_text(
                "meow This federation does not exist.")
            return
        ...
        update.effective_message.reply_text("meow What should I delete?")
        return

    if is_user_fed_owner(fed_id, user.id) is False:
        update.effective_message.reply_text(
            "meow Only federation owners can do this!")
        return

    update.effective_message.reply_text(
        "meow You sure you want to delete your federation? This cannot be reverted, you will lose your entire ban list, and '{}' will be permanently lost."
...
def fed_chat(update: Update, context: CallbackContext):
    ...
    if not fed_id:
        update.effective_message.reply_text(
            "meow This group is not in any federation!")
        return
...
def join_fed(update: Update, context: CallbackContext):
    ...
    if chat.type == 'private':
        send_message(update.effective_message,
                     "meow This command is specific to the group, not to our pm!")
        return
        ...
        for admin in administrators:
            ...
            if status == "creator":
                if str(admin.user.id) == str(user.id):
                    ...
                    update.effective_message.reply_text(
                        "meow Only group creators can use this command!")
                    return
    if fed_id:
        message.reply_text("meow You cannot join two federations from one chat")
        return

    if len(args) >= 1:
        getfed = sql.search_fed_by_id(args[0])
        if getfed is False:
            message.reply_text("meow Please enter a valid federation ID")
            return

        x = sql.chat_join_fed(args[0], chat.title, chat.id)
        if not x:
            message.reply_text(
                "meow Failed to join federation! Please contact @OnePunchSupport should this problem persist!"
            )
            return
        ...
        message.reply_text("meow This group has joined the federation: {}!".format(
            getfed['fname']))
...
def user_join_fed(update: Update, context: CallbackContext):
    ...
    if is_user_fed_owner(fed_id, user.id) or user.id in DRAGONS:
        ...
        if user_id:
            ...
            msg.reply_text("meow I cannot extract user from this message")
            return
        else:
        ...
        if getuser:
            update.effective_message.reply_text(
                "sed life meow I cannot promote users who are already federation admins! Can remove them if you want!"
            )
            return
        if user_id == bot.id:
            update.effective_message.reply_text(
                "meow I already am a federation admin in all federations!")
            return
        res = sql.user_join_fed(fed_id, user_id)
        if res:
            update.effective_message.reply_text("meow Successfully Promoted!")
        else:
            update.effective_message.reply_text("Failed to promote meow!")
    else:
        update.effective_message.reply_text(
            "meow Only federation owners can do this!")
