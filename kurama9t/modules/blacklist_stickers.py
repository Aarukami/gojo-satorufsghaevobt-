...
import kurama9tAnimeBot.modules.sql.blsticker_sql as sql
from kurama9tAnimeBot import LOGGER, dispatcher
...
from kurama9tAnimeBot.modules.helper_funcs.chat_status import (user_admin,
                                                           user_not_admin)
from kurama9tAnimeBot.modules.helper_funcs.misc import split_message
from kurama9tAnimeBot.modules.helper_funcs.string_handling import extract_time

from kurama9tAnimeBot.modules.log_channel import loggable
from kurama9tAnimeBot.modules.warns import warn
...
def add_blackliststicker(update: Update, context: CallbackContext):
    ...
    if len(words) > 1:
        ...
        send_message(
            update.effective_message,
            "meow this Sticker <code>{}</code>is added to blacklist stickers in <b>{}</b>!"
            .format(trigger, html.escape(chat_name)),
            parse_mode=ParseMode.HTML)
    else:
        send_message(update.effective_message,
                     "meow Tell me what stickers you want to add to the blacklist.")
    ...
    if len(words) > 1:
        ...
        if len(to_unblacklist) == 1:
            if successful:
                send_message(
                    update.effective_message,
                    "meow this Sticker <code>{}</code>is deleted from blacklist in <b>{}</b>!"
                    .format(
                        html.escape(to_unblacklist[0]), html.escape(chat_name)),
                    parse_mode=ParseMode.HTML)
            else:
                send_message(update.effective_message,
                             "meow This sticker is not on the blacklist...!")
            ...
            send_message(
                update.effective_message,
                "meow None of these stickers exist, so they cannot be removed."
                .format(successful,
        ...
        trigger = msg.reply_to_message.sticker.set_name
        if trigger is None:
            send_message(update.effective_message, "meow Sticker is invalid!")
            return
        success = sql.rm_from_stickers(chat_id, trigger.lower())
...
def blacklist_mode(update: Update, context: CallbackContext):
    ...
    if conn:
        ...
        if update.effective_message.chat.type == "private":
            send_message(update.effective_message,
                         "meow You can do this command in groups, not PM")
            return ""
