...
from kurama9tAnimeBot import dispatcher, LOGGER, DRAGONS
from kurama9tAnimeBot.modules.disable import DisableAbleCommandHandler
from kurama9tAnimeBot.modules.helper_funcs.handlers import MessageHandlerChecker
from kurama9tAnimeBot.modules.helper_funcs.chat_status import user_admin
from kurama9tAnimeBot.modules.helper_funcs.extraction import extract_text
from kurama9tAnimeBot.modules.helper_funcs.filters import CustomFilters
from kurama9tAnimeBot.modules.helper_funcs.misc import build_keyboard_parser
from kurama9tAnimeBot.modules.helper_funcs.msg_types import get_filter_type
from kurama9tAnimeBot.modules.helper_funcs.string_handling import (
    split_quotes,
    button_markdown_parser,
    escape_invalid_curly_brackets,
    markdown_to_html,
)
from kurama9tAnimeBot.modules.sql import cust_filters_sql as sql

from kurama9tAnimeBot.modules.connection import connected

from kurama9tAnimeBot.modules.helper_funcs.alternate import send_message, typing_action
...
def stop_filter(update, context):
    ...
    if len(args) < 2:
        send_message(update.effective_message, "meow What should i stop?")
        return

    chat_filters = sql.get_chat_triggers(chat_id)

    if not chat_filters:
        send_message(update.effective_message, "sorry meow but No filters active here!")
        return
    ...
    for keyword in chat_filters:
        if keyword == args[1]:
            ...
            send_message(
                update.effective_message,
                "Okay meow, I'll stop replying to that filter in *{}*.".format(
                    chat_name),
                parse_mode=telegram.ParseMode.MARKDOWN,
            )
            raise DispatcherHandlerStop
    ...
    send_message(
        update.effective_message,
        "That's not a filter meow - Click: /filters to get currently active filters.",
    )
