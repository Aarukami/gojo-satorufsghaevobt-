...
from kurama9tAnimeBot import (DEV_USERS, LOGGER, OWNER_ID, DRAGONS, DEMONS, TIGERS,
                          WOLVES, dispatcher)
from kurama9tAnimeBot.modules.disable import DisableAbleCommandHandler
from kurama9tAnimeBot.modules.helper_funcs.chat_status import (
    bot_admin, can_restrict, connection_status, is_user_admin,
    is_user_ban_protected, is_user_in_chat, user_admin, user_can_ban)
from kurama9tAnimeBot.modules.helper_funcs.extraction import extract_user_and_text
from kurama9tAnimeBot.modules.helper_funcs.string_handling import extract_time
from kurama9tAnimeBot.modules.log_channel import gloggable, loggable
...
"""
    ...
    if not user_id:
        message.reply_text("Sed life I doubt that's a user.")
        return log_message

    try:
        member = chat.get_member(user_id)
    except BadRequest as excp:
        if excp.message == "Sed User not found":
            message.reply_text("Sed life I can't find this user.")
            return log_message
        else:
            raise

    if user_id == bot.id:
        message.reply_text("Sed life How would I unban myself if I wasn't here...?")
        return log_message

    if is_user_in_chat(chat, user_id):
        message.reply_text("Sed life Isn't this person already here??")
        return log_message

    chat.unban_member(user_id)
    message.reply_text("Bery Noice now this user can join again!")
...
def ban(update: Update, context: CallbackContext) -> str:
    ...
    if not user_id:
        message.reply_text("My meowsense says this is not a User.")
        return log_message
    ...
    try:
        ...
        if excp.message == "User not found":
            message.reply_text("Sed life i Can't find this person.")
            return log_message
        else:
            raise

    if user_id == bot.id:
        message.reply_text("you want me to ban myself if u ban me i will curse you tht u will die single!")
        return log_message

    if is_user_ban_protected(chat, user_id, member) and user not in DEV_USERS:
        if user_id == OWNER_ID:
            message.reply_text(
                "Sed life he is an admin meow cant do anything")
            return log_message
        elif user_id in DEV_USERS:
            message.reply_text("sed life meow can't act against his own friend")
            return log_message
            ...
            message.reply_text("its Itachi you baka i cant ban him .")
            return log_message
...
def kick(update: Update, context: CallbackContext) -> str:
    ...
    if res:
        ...
        bot.sendMessage(
            chat.id,
            f"User Kicked Woops meow bad! {mention_html(member.user.id, html.escape(member.user.first_name))}.",
            parse_mode=ParseMode.HTML)
        log = (
            f"<b>{html.escape(chat.title)}:</b>\n"
            f"#KICKED\n"
...
def kickme(update: Update, context: CallbackContext):
    ...
    if is_user_admin(update.effective_chat, user_id):
        update.effective_message.reply_text(
            "sed life meow u are an admin so i cant kick u")
        return
...
def unban(update: Update, context: CallbackContext) -> str:
    ...
    chat.unban_member(user_id)
    message.reply_text("oke doke meow, this user can join!")
...
def selfunban(context: CallbackContext, update: Update) -> str:
    ...
    try:
        ...
        if excp.message == "User not found":
            message.reply_text("I can't seem to find this meow.")
            return
        else:
            raise

    if is_user_in_chat(chat, user.id):
        message.reply_text("Aren't you already in the chat meow??")
        return

    chat.unban_member(user.id)
    message.reply_text("Yep, I have unbanned you meow.")
