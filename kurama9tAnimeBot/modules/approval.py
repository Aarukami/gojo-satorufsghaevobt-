...
from kurama9tAnimeBot import dispatcher, DRAGONS
...
import kurama9tAnimeBot.modules.sql.approve_sql as sql
...
def approve(update, context):
    ...
    if not user_id:
        message.reply_text(
            "sed life,I don't know who you're talking about, you're going to need to specify a user!"
        )
        return ""
    try:
    ...
    if member.status == "administrator" or member.status == "creator":
        message.reply_text(
            "Sed life User is already admin - locks, blocklists, and antiflood already don't apply to them."
        )
        return ""
